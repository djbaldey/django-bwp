...
def serialize(objects, **options):
    """
    Сериализатор в объект(ы) Python, принимает либо один экземпляр
    Django-модели, либо QuerySet или объект паджинации
    и точно также возвращает.
    """
    return serializers.serialize('python', objects, **options)
