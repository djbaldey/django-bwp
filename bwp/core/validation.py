...
from django.forms.models import (BaseModelForm, BaseModelFormSet, fields_for_model,
    _get_foreign_key)
from bwp.core import ListFilter, FieldListFilter
from bwp.core.util import get_fields_from_path, NotRelationField
from bwp.core.options import (flatten_fieldsets, BaseModelBWP,
    HORIZONTAL, VERTICAL)
...
def validate(cls, model):
    """
    Does basic ModelBWP option validation. Calls custom validation
    ...
    if hasattr(cls, 'list_editable') and cls.list_editable:
        ...
        for idx, field_name in enumerate(cls.list_editable):
            ...
            if not field.editable:
                raise ImproperlyConfigured("'%s.list_editable[%d]' refers to a "
                    "field, '%s', which isn't editable through the bwp."
                    % (cls.__name__, idx, field_name))
    ...
    if hasattr(cls, 'inlines'):
        ...
        for idx, inline in enumerate(cls.inlines):
            if not issubclass(inline, BaseModelBWP):
                raise ImproperlyConfigured("'%s.inlines[%d]' does not inherit "
                        "from BaseModelBWP." % (cls.__name__, idx))
            if not inline.model:
...
def validate_fields_spec(cls, model, opts, flds, label):
    """
    Validate the fields specification in `flds` from a ModelBWP subclass
    ...
    if hasattr(cls, 'radio_fields'):
        ...
        for field, val in cls.radio_fields.items():
            ...
            if not val in (HORIZONTAL, VERTICAL):
                raise ImproperlyConfigured("'%s.radio_fields['%s']' "
                        "is neither bwp.HORIZONTAL nor bwp.VERTICAL."
                        % (cls.__name__, field))
